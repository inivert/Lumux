...
export async function GET() {
  try {
    ...
    // Transform products
    const transformedProducts = stripeProducts.data
      .map(product => {
        try {
      ...
      .filter((product): product is NonNullable<TransformedProduct> => {
    ...
    const sortedProducts = [...transformedProducts].sort((a, b) => {
      ...
      if (aName === 'BUY A WEBSITE') return -1;
      if (bName === 'BUY A WEBSITE') return 1;
      if (aName === 'STARTER PLAN') return -1;
      if (bName === 'STARTER PLAN') return 1;
      return 0;
    });
